app.controller('exchangeGroupController', function ($scope, $rootScope, $route, $routeParams, $location, exchangeGroupService, alertsService) {
    $scope.initializeController = function () {

        var exchangeGroupID = ($routeParams.id || "");
        $rootScope.applicationModule = "exchangeGroups";
        $rootScope.alerts = [];

        $scope.initObject();
        exchangeGroupService.getDataParent($scope.getDataParentCompleted, $scope.getError);

        if (exchangeGroupID > 0) {
            var obj = new Object();
            obj.Id = exchangeGroupID;
            exchangeGroupService.getexchangeGroup(obj, $scope.getEditCompleted, $scope.getError);
        }
        else {
            exchangeGroupService.getIndex($scope.getIndexCompleted, $scope.getError);
        }
    ...
    $scope.create = function () {
        var exchangeGroup = $scope.createObject();
        exchangeGroupService.create(exchangeGroup, $scope.createCompleted, $scope.createError);
    }
    $scope.createBack = function () {
        var exchangeGroup = $scope.createObject();
        exchangeGroupService.create(exchangeGroup, $scope.createBackCompleted, $scope.createError);
    }
    ...
    $scope.update = function () {
        var exchangeGroup = $scope.createObject();
        exchangeGroupService.update(exchangeGroup, $scope.updateCompleted, $scope.updateError);
    }
    $scope.updateBack = function () {
        var exchangeGroup = $scope.createObject();
        exchangeGroupService.update(exchangeGroup, $scope.updateBackCompleted, $scope.updateError);
    }
    ...
    $scope.deletes = function (arr) {
        exchangeGroupService.deletes(arr, $scope.deletesCompleted, $scope.deletesError);
    }
