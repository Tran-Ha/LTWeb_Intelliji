app.directive('ngJqgrid', function ($window) {
    return {
        restrict: 'E',
        replace: true,
        scope: {
...
app.directive('optionsDisabled', ['$parse', function ($parse) {
    ...
    .directive('ngTag', [function () {
        return function (scope, element, attrs) {
            element.bind("keydown keypress", function (event) {
                var keycode = event.keyCode || event.which;
                if (keycode === 13 || keycode == 9 || keycode == 44) {

                    if (scope.$root.$$phase != '$apply' && scope.$root.$$phase != '$digest') {
                        //scope.$apply();
                        scope.$apply(function () {
                            scope.$eval(attrs.ngTag);
                        });
                    }
                    event.preventDefault();
                }
...
app.factory('youtubeEmbed', ['$document', '$q', '$rootScope', function ($document, $q, $rootScope) {
...
app.directive('youtube', ['youtubeEmbed', '$window', function (youtubeEmbed, $window) {
    return {
        restrict: 'E',
