app.controller('galleryGroupController', function ($scope, $rootScope, $route, $routeParams, $location, galleryGroupService, alertsService) {
    $scope.initializeController = function () {

        var galleryGroupID = ($routeParams.id || "");
        $rootScope.applicationModule = "galleryGroups";
        $rootScope.alerts = [];

        $scope.initObject();
        galleryGroupService.getDataParent($scope.getDataParentCompleted, $scope.getError);

        if (galleryGroupID > 0) {
            var obj = new Object();
            obj.Id = galleryGroupID;
            galleryGroupService.getgalleryGroup(obj, $scope.getEditCompleted, $scope.getError);
        }
        else {
            galleryGroupService.getIndex($scope.getIndexCompleted, $scope.getError);
        }
    ...
    $scope.create = function () {
        var galleryGroup = $scope.createObject();
        galleryGroupService.create(galleryGroup, $scope.createCompleted, $scope.createError);
    }
    $scope.createBack = function () {
        var galleryGroup = $scope.createObject();
        galleryGroupService.create(galleryGroup, $scope.createBackCompleted, $scope.createError);
    }
    ...
    $scope.update = function () {
        var galleryGroup = $scope.createObject();
        galleryGroupService.update(galleryGroup, $scope.updateCompleted, $scope.updateError);
    }
    $scope.updateBack = function () {
        var galleryGroup = $scope.createObject();
        galleryGroupService.update(galleryGroup, $scope.updateBackCompleted, $scope.updateError);
    }
    ...
    $scope.deletes = function (arr) {
        galleryGroupService.deletes(arr, $scope.deletesCompleted, $scope.deletesError);
    }
